...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class DialogueSystem1xTo2xUpdater
    {
        ...
        private static void UpdateNow()
        {
            ...
            // Visible meta files:
            var originalExternalVersionControl = VersionControlSettings.mode;
            if (!VersionControlSettings.mode.Equals(ExternalVersionControlVisibleMetaFiles))
            {
                ...
                VersionControlSettings.mode = ExternalVersionControlVisibleMetaFiles;
                if (cancel)
                {
                    Debug.Log("Dialogue System: Update process cancelled. Serialization mode is " + EditorSettings.serializationMode + " and Version Control Mode is " +
                        VersionControlSettings.mode + ". To change these settings, use menu item Edit > Project Settings > Editor.");
                    return;
                }
            }
            ...
            finally
            {
                ...
                if (!cancel)
                {
                    ...
                    if (originalExternalVersionControl != ExternalVersionControlVisibleMetaFiles)
                    {
                        ...
                        VersionControlSettings.mode = originalExternalVersionControl;
                    }
